...
既に ON の状態で誤って OFF にすることはありません。

実機（CDP 経由で本物の会議に接続）で確認したところ、`element.click()` は**約 1 秒で反映**されました。
`focus()` + Enter キーの合成イベントは無反応でした（12 秒待っても変化なし）ため、`click()` を使っています。
念のため自動実行は **8 秒間隔・最大 2 回**にしています（ON になればラベルが「オフにする」に変わり、
探索対象から外れるのでそれ以上は押しません）。手動で押し直したいときはツールバーの `字幕ON` を使います。
...
4. **`role=region` による推測はしません**。実機で「会議の詳細」など無関係なリージョンを掴み、
   静的テキストを発話として表示したうえ、「コンテナが見つかった」判定で CC の自動 ON まで
   抑止してしまったためです。見つからない場合は素直に案内を出します
5. 上記のうち曖昧な経路（`aria-live` フォールバック）で掴んだ要素から発話が取れないときは、
   その要素を候補から除外して再検出します
6. 字幕が OFF（=「字幕をオンにする」ボタンが存在する）間は曖昧なフォールバックを使いません。
   無関係な要素を掴んで CC の自動 ON を妨げるのを防ぐためです
7. **発話行**: コンテナ内に（アナウンサーでない）`aria-live` があればそれを、無ければコンテナ自身を起点に、
...
8. **話者名と本文**: 行内の「本文テキストを持つ末端要素」を文書順に集め、先頭要素が
...
| CC ボタンの自動 ON | 成功。字幕 OFF から起動 → `element.click()` が約 1 秒で反映 → リージョン出現 → 検出まで通過 |
| 連打による ON→OFF 戻し | 発生せず（自動 1/2 回で ON になり、以降は押さない） |
| 実際の発話の取り込み | 成功。実会議で 60 秒間の日本語字幕を取り込み、Meet 側 315 文字とオーバーレイ 315 文字が**完全一致**（末尾テキストも同一） |
| 字幕 OFF からの通し確認 | 成功。`字幕をオンにする` を自動クリック → 1 秒で ON → 検出 → 5 秒後には発話を表示 |
...
| `?ccoff&nobutton` | 字幕 OFF かつ CC ボタンが見つからない最悪ケースで、無関係なリージョンを掴まず案内を出すか |
