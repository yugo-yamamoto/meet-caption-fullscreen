...
実機（CDP 経由で本物の会議 DOM を観測）では、このボタンのクリックが Meet 側に反映されるまで
数秒かかることがありました。短い間隔で押すと ON にした直後に OFF へ戻してしまうため、
自動実行は **8 秒間隔・最大 2 回**にしています（ON になればラベルが「オフにする」に変わり、
探索対象から外れるのでそれ以上は押しません）。手動で押し直したいときはツールバーの `字幕ON` を使います。

> [!WARNING]
> 合成イベント（`element.click()` など）で Meet の CC ボタンを操作できるかは**まだ確証がありません**。
> 実機では反応しないケースを観測しており、その場合は自動 ON が機能せず、案内ログのみ出ます。
> 字幕は手動で ON にしてから使ってください（検出・表示・履歴は問題なく動作します）。
...
| CC ボタンの自動 ON | **未確定**（合成イベントが反映されないケースを観測） |

字幕を OFF にしても `[role="region"][aria-label="字幕"]` 自体は DOM に残り、テキストだけが空になります。
また字幕 OFF のとき `[role="region"][aria-label="通話コントロール"]` が最大のテキスト量を持つため、
かつての `role=region` フォールバックはこれを掴んでいました（撤去済み）。
